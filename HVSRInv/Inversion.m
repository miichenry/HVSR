function [parameters] = Inversion(sta_name, LayerNum,InvNum,IterNum,PopNum,GeneNum,Freq,HVSRData,FreqMin,FreqMax,SampleNum,InitModData,Handle)
    ...
    for j=1:InvNum    
        ...
        for jm=1:IterNum        
            ...
            if jm==IterNum                
                ...
                hold on
                fig1 = figure(2)
                plot(0:IterNum-1,bestFit,'k','LineWidth',1),hold on;
                plot(jm2,bestFit(jm2),'ro','MarkerFaceColor','r','MarkerSize',10)
                xlim([0 IterNum]);
                
                
                title(['Station ' sta_name ' - Inversion No: ' num2str(j)])
                xlabel('Generations');
                ylabel('RMSE');
                ...
                saveas(fig1, fullfile('Outputs', [sta_name '_bestFit.png']));
